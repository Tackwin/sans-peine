...
static int cursor_to_idx(
    const ImVec2& pos, const ImRect& bb, const PlotConfig& conf, float x_min, float x_max
) {
    const float t = ImClamp((pos.x - bb.Min.x) / (bb.Max.x - bb.Min.x), 0.0f, 0.9999f);
    auto inv = rescale_inv(t, x_min, x_max, conf.scale.type) * (conf.values.count - 1);
    const int v_idx = (int)std::round(inv);
...
PlotStatus Plot(const char* label, const PlotConfig& conf) {
    ...
    if (conf.values.count > 0) {
        ...
        float x_max = (float)conf.values.offset + conf.values.count - 1.f;
        if (conf.values.xs) {
        ...
        if (conf.tooltip.show && hovered && inner_bb.Contains(g.IO.MousePos)) {
            ...
            std::string format;
            format.reserve((strlen(conf.tooltip.format) + 1) * ys_count);
            ...
            for (size_t i = 0; i < ys_count; ++i) {
                if (conf.tooltip.ys_names) {
                    ...
                    snprintf(temp_buffer, 256, conf.tooltip.format, x0, ys_list[i][data_idx]);
                }
                format += temp_buffer;
                format += '\n';
            }
        ...
        if (conf.grid_y.show) {
            ...
            float cnt =
                (conf.scale.max - conf.scale.min) / (conf.grid_y.size / conf.grid_y.subticks);
            float inc = 1.f / cnt;
            for (int i = 0; i <= cnt; ++i) {
